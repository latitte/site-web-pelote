...
foreach ($permanences as $permanence) {
    $jours = explode(', ', $permanence); // Notez l'espace après la virgule pour correspondre au format enregistré
    foreach ($jours as $jour) {
        $joursPris[] = trim($jour); // Utiliser trim pour éviter les espaces blancs
    }
}
...
for ($jour = 1; $jour <= 31; $jour++) {
    ...
    if ($jourSemaine < 6 && !in_array($date, $joursPris)) {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                flatpickr("#date", {
                    ...
                    onChange: function(selectedDates) {
                        ...
                        if (selectedDate && joursPris.includes(selectedDate.toISOString().split('T')[0])) {
